...
import { CollisionTypeModalComponent } from 'src/app/shared/collision-type-modal/collision-type-modal.component';

@Component({
  selector: 'app-collision-type',
  templateUrl: './collision-type.component.html',
  styleUrls: ['./collision-type.component.scss']
})
export class CollisionTypeComponent implements OnInit {
  accidentCauses:CollisionType[]=[];
  constructor(private sharedbuttonService: SharedButtonLabelService, private notification:NzNotificationService, private languageService:LanguageService,private fb: FormBuilder,private modal: NzModalService,private vechileService:CollisionTypeService) {}
  ...
  openDeleteConfirmation(collisionTypeId: number) {
    this.modal.confirm({
      ...
      nzOnOk: () => {
        this.deleteVechile(collisionTypeId);
      },
      nzOnCancel: () => {
  ...
  deleteVechile(collisionTypeId:number)
{
  this.vechileService.delete(collisionTypeId,'collisionTypeId').subscribe(
    (response) => {
      // Success logic, if needed
      // Remove the deleted region from the regions array
      const updatedVechiles = this.accidentCauses.filter(vechile => vechile.collisionTypeId !== collisionTypeId);
  ...
  editRegion(collisionTypeId: number) {
    this.vechileService.update(collisionTypeId);
  }
  ...
  showModal(action: string): void {
    ...
    const modalRef = this.modal.create({
      ...
      nzContent: CollisionTypeModalComponent,
      nzFooter: null,
      nzOnOk: () => {
      ...
      } as Partial<CollisionTypeModalComponent> // Type assertion to Partial<SharedModalComponent>
