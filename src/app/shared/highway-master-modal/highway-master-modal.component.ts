...
@Component({
  selector: 'app-highway-master-modal',
  templateUrl: './highway-master-modal.component.html',
  styleUrls: ['./highway-master-modal.component.scss']
})
export class HighwayMasterModalComponent implements OnInit{
  ...
  constructor(
    ...
    private healthService: HighwayMasterService
  ) { }
  ...
  ngOnInit(): void {
    ...
    .subscribe((rowData: any) => {
      ...
      this.validateForm.patchValue({
        hname: rowData.hname,
        hnameAm:rowData.hnameAm,
        hnameOr:rowData.hnameOr,
        hnameTi:rowData.hnameTi,
        hnameAf:rowData.hnameAf,
        hnameSo:rowData.hnameSo,
  ...
  setButtonLable()
{
  this.sharedbuttonService.getButtonLabel().subscribe((label: string) => {
    if (this.action === 'Edit') {
      ...
      this.buttonLabel = 'Update high way'; // Customize the label for editing
      this.onUpdate();
