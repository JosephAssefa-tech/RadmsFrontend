...
@Component({
  selector: 'app-vechile-master',
  templateUrl: './vechile-master.component.html',
  styleUrls: ['./vechile-master.component.scss']
})
export class VechileMasterComponent implements OnInit {
  vechiles: VechicleMasterEntity[]=[];
  constructor(private sharedbuttonService: SharedButtonLabelService, private notification:NzNotificationService, private languageService:LanguageService,private fb: FormBuilder,private modal: NzModalService,private vechileService:VechileService) {}
  ...
  deleteVechile(vehicleId:number)
{
  this.vechileService.delete(vehicleId,'vehicleId').subscribe(
    (response) => {
      // Success logic, if needed
      // Remove the deleted region from the regions array
      const updatedVechiles = this.vechiles.filter(vechile => vechile.vehicleId !== vehicleId);
      this.vechiles = updatedVechiles;
    },
    (error) => {
  ...
  showModal(action: string): void {
    ...
    const modalRef = this.modal.create({
      ...
      nzContent: VechileMasterModalComponent,
      nzFooter: null,
      nzOnOk: () => {
      ...
      } as Partial<VechileMasterModalComponent> // Type assertion to Partial<SharedModalComponent>
