...
@Component({
  ...
  templateUrl: './victim-type.component.html',
  styleUrls: ['./victim-type.component.scss']
})
export class VictimTypeComponent implements OnInit {

  accidentCauses:VictimTypeLookup[]=[];
  constructor(private sharedbuttonService: SharedButtonLabelService, private notification:NzNotificationService, private languageService:LanguageService,private fb: FormBuilder,private modal: NzModalService,private vechileService:VictimTypeService) {}
  ...
  openDeleteConfirmation(victimTypeId: number) {
    this.modal.confirm({
      ...
      nzOnOk: () => {
        this.deleteVechile(victimTypeId);
      },
      nzOnCancel: () => {
  ...
  deleteVechile(victimTypeId:number)
{
  this.vechileService.delete(victimTypeId,'victimTypeId').subscribe(
    (response) => {
      // Success logic, if needed
      // Remove the deleted region from the regions array
      const updatedVechiles = this.accidentCauses.filter(vechile => vechile.victimTypeId !== victimTypeId);
  ...
  editRegion(victimTypeId: number) {
    this.vechileService.update(victimTypeId);
  }
